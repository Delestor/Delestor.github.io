...
function getParams(startAttr,speedAttr){
    ...
    for(var i=0;i<params.length;i++){
        ...
        if(params[i].getAttribute(speedAttr)!==null&&params[i].getAttribute(speedAttr)!==""){speed=params[i].getAttribute(speedAttr);
    }
    return;
}
}
...
window.onload=function(e){
    ...
    var title=title_ref.text;var i=0;
    
    setTimeout(function(){
        setInterval(function(){
            title_ref.text=title.substr(i,title.length)+"  ---  "+title.substr(0,i);i++;if(i===title.length){
                i=0;
            }
        },speed);
    },start);
